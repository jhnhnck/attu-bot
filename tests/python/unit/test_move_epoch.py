...
from nova_core.client.calendar import move_epoch
...
class TestMoveEpochYearUpdates:
    ...
    async def test_epoch_extend_adds_note(self, make_year):
        ...
        with (
            patch('nova_core.client.years.Year.get', new_callable=AsyncMock) as mock_get,
            patch('nova_core.client.years.Year.update', new_callable=AsyncMock) as mock_update,
            patch('nova_core.client.calendar.get_year_status') as mock_status,
            patch('nova_core.client.calendar.get_year_span', new_callable=AsyncMock) as mock_span,
            patch('nova_core.client.calendar.config') as mock_config,
        ):
    ...
    async def test_no_crash_if_year_record_missing(self):
        ...
        with patch('nova_core.client.years.Year.get', new_callable=AsyncMock) as mock_get, patch('nova_core.client.calendar.get_year_status') as mock_status, patch('nova_core.client.calendar.get_year_span', new_callable=AsyncMock) as mock_span, patch('nova_core.client.calendar.config') as mock_config:
