...
class TestPingCommand:
    ...
    async def test_ping_responds_pong(self, mock_ctx):
        ...
        from doom_bot import command_ping
...
class TestPongCommand:
    ...
    async def test_pong_owner_immediate_response(self, mock_ctx_factory):
        ...
        from doom_bot.commands.debug import command_pong

        ctx = mock_ctx_factory(user_id=999, is_owner=True)
        # Mock config.is_owner to return True
        with patch('doom_bot.commands.debug.config.is_owner', return_value=True):
    ...
    async def test_pong_non_owner_ping_response(self, mock_ctx_factory):
        ...
        from doom_bot import tasks as tasks_module
        from doom_bot.commands.debug import command_pong
        ...
        with patch('doom_bot.commands.debug.config.is_owner', return_value=False), patch.object(tasks_module, 'scheduler', mock_scheduler):
            await command_pong(ctx)
