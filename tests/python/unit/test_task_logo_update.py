...
from attubot.tasks.logo_update import LogoUpdateTask
...
class TestOnStart:
    async def test_awaits_config_and_bot_then_sleeps(self):
        ...
        with patch('attubot.tasks.logo_update.config', mock_cfg), patch('attubot.tasks.logo_update.bot', mock_bot), patch('attubot.tasks.logo_update.asyncio.sleep', new_callable=AsyncMock) as mock_sleep:
            await task.on_start()
...
class TestRunHappyPath:
    async def test_calculates_rotation_and_updates_avatar(self):
        ...
        with (
            patch('attubot.tasks.logo_update.config', mock_cfg),
            patch('attubot.tasks.logo_update.bot', mock_bot),
            patch('attubot.tasks.logo_update.get_year_status', return_value=(100, 5)),
            patch('attubot.tasks.logo_update.get_year_span', new_callable=AsyncMock, return_value=year_span),
            patch('attubot.tasks.logo_update.generate_png', new_callable=AsyncMock, return_value=b'\x89PNG') as mock_png,
            patch('attubot.tasks.logo_update.logger') as mock_logger,
        ):
    ...
    async def test_rotation_stored_modulo_360(self):
        ...
        with (
            ...
            patch('attubot.tasks.logo_update.get_year_status', return_value=(100, 5)),
            patch('attubot.tasks.logo_update.get_year_span', new_callable=AsyncMock, return_value=year_span),
            patch('attubot.tasks.logo_update.generate_png', new_callable=AsyncMock, return_value=b'\x89PNG'),
            patch('attubot.tasks.logo_update.logger') as mock_logger,
        ):
            mock_logger.send_to_webhook = AsyncMock()
            await task.run()
...
class TestRunEpochPaused:
    async def test_uses_random_drift_when_paused(self):
        ...
        with (
            patch('attubot.tasks.logo_update.config', mock_cfg),
            patch('attubot.tasks.logo_update.bot', mock_bot),
            patch('attubot.tasks.logo_update.random', return_value=0.8),
            patch('attubot.tasks.logo_update.generate_png', new_callable=AsyncMock, return_value=b'\x89PNG'),
            patch('attubot.tasks.logo_update.logger') as mock_logger,
        ):
            mock_logger.send_to_webhook = AsyncMock()
...
class TestColorMath:
    ...
    async def test_computed_color_matches_expected_for_known_rotation(self):
        ...
        with (
            patch('attubot.tasks.logo_update.config', mock_cfg),
            patch('attubot.tasks.logo_update.bot', mock_bot),
            patch('attubot.tasks.logo_update.random', return_value=0.0),
            patch('attubot.tasks.logo_update.generate_png', new_callable=AsyncMock, return_value=b'\x89PNG') as mock_png,
            patch('attubot.tasks.logo_update.logger') as mock_logger,
        ):
            mock_logger.send_to_webhook = AsyncMock()
...
class TestRunGuildAvatarUpdate:
    async def test_guild_edit_called_with_icon(self):
        ...
        with patch('attubot.tasks.logo_update.config', mock_cfg), patch('attubot.tasks.logo_update.bot', mock_bot), patch('attubot.tasks.logo_update.random', return_value=0.0), patch('attubot.tasks.logo_update.generate_png', side_effect=fake_generate_png), patch('attubot.tasks.logo_update.logger') as mock_logger:
            mock_logger.send_to_webhook = AsyncMock()
            await task.run()
...
class TestRunThemePersistence:
    async def test_theme_rotation_and_color_saved(self):
        ...
        with (
            patch('attubot.tasks.logo_update.config', mock_cfg),
            patch('attubot.tasks.logo_update.bot', mock_bot),
            patch('attubot.tasks.logo_update.random', return_value=0.5),
            patch('attubot.tasks.logo_update.generate_png', new_callable=AsyncMock, return_value=b'\x89PNG'),
            patch('attubot.tasks.logo_update.logger') as mock_logger,
        ):
            mock_logger.send_to_webhook = AsyncMock()
...
class TestSingleton:
    def test_module_level_singleton_exists(self):
        ...
        from attubot.tasks.logo_update import logo_update_task
