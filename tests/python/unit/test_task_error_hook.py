...
from attubot.tasks.error_hook import ErrorHookTask, error_hook_refresh
...
class TestOnStart:
    async def test_awaits_config_ready_and_bot_ready(self):
        ...
        with patch('attubot.tasks.error_hook.config', mock_cfg), patch('attubot.tasks.error_hook.bot', mock_bot):
            await task.on_start()
...
class TestRunTestMode:
    async def test_skips_when_test_mode(self):
        ...
        with patch('attubot.tasks.error_hook.config', mock_cfg), patch('attubot.tasks.error_hook.bot', mock_bot), patch('attubot.tasks.error_hook.logger') as mock_logger:
...
class TestRunWebhookCreationWithAvatar:
    async def test_generate_png_called_with_correct_args(self):
        ...
        with patch('attubot.tasks.error_hook.config', mock_cfg), patch('attubot.tasks.error_hook.bot', mock_bot), patch('attubot.tasks.error_hook.generate_png', new_callable=AsyncMock, return_value=icon_bytes) as mock_png, patch('attubot.tasks.error_hook.logger') as mock_logger:
            mock_logger.send_to_webhook = AsyncMock()
            await task.run()
...
class TestSingleton:
    def test_module_level_singleton_exists(self):
        ...
        from attubot.tasks.error_hook import error_hook_task
