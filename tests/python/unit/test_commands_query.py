...
class TestQueryPinsCommand:
    ...
    async def test_pins_no_results(self, mock_ctx, guild):
        ...
        from attubot.commands.query import query_pins
        ...
        with patch('attubot.commands.query.get_year_status', return_value=(7, 1)), patch('attubot.commands.query.get_year_span', new_callable=AsyncMock, return_value=mock_span), patch('attubot.commands.query.snowflake_time', return_value=datetime(2020, 1, 1)):
    ...
    async def test_pins_single_pin_found(self, mock_ctx, guild):
        ...
        with (
            patch('attubot.commands.query.get_year_status', return_value=(7, 1)),
            patch('attubot.commands.query.get_year_span', new_callable=AsyncMock, return_value=mock_span),
            patch('attubot.commands.query.snowflake_time', return_value=datetime(2020, 1, 1)),
            patch('attubot.commands.query.format_message_link', return_value='https://discord.com/channels/123/456/100'),
        ):
            await query_pins(mock_ctx, channel=mock_channel)
    ...
    async def test_pins_multiple_pins_found(self, mock_ctx, guild):
        ...
        with (
            ...
            patch('attubot.commands.query.get_year_span', new_callable=AsyncMock, return_value=mock_span),
            patch('attubot.commands.query.snowflake_time', return_value=datetime(2020, 1, 1)),
            patch('attubot.commands.query.format_message_link', side_effect=[f'https://discord.com/channels/123/456/{100 + i}' for i in range(3)]),
        ):
            await query_pins(mock_ctx, channel=mock_channel)
    ...
    async def test_pins_multiple_years(self, mock_ctx, guild):
        ...
        with (
            patch('attubot.commands.query.get_year_status', return_value=(7, 2)),
            patch('attubot.commands.query.get_year_span', new_callable=AsyncMock, return_value=mock_span),
            patch('attubot.commands.query.snowflake_time', return_value=datetime(2020, 1, 1)),
            patch('attubot.commands.query.format_message_link', return_value='https://discord.com/channels/123/456/100'),
        ):
    ...
    async def test_pins_message_reference_without_id(self, mock_ctx, guild):
        ...
        with (
            ...
            patch('attubot.commands.query.get_year_span', new_callable=AsyncMock, return_value=mock_span),
            patch('attubot.commands.query.snowflake_time', return_value=datetime(2020, 1, 1)),
            patch('attubot.commands.query.format_message_link', return_value='https://discord.com/channels/123/456/0') as mock_format,
        ):
            await query_pins(mock_ctx, channel=mock_channel)
