...
from attubot.tasks.presence import PresenceUpdateTask
...
class TestOnStart:
    async def test_awaits_config_wait_for_ready(self):
        ...
        with patch('attubot.tasks.presence.config', mock_cfg):
            await task.on_start()
...
class TestRunHappyPath:
    async def test_fetches_count_and_sets_presence(self):
        ...
        with (
            patch('attubot.tasks.presence.bot', mock_bot),
            patch('attubot.eggs.hatching._egg_repo', mock_egg_repo),
            patch('attubot.tasks.presence.logger') as mock_logger,
        ):
...
class TestRunRepoNone:
    async def test_returns_early_when_egg_repo_is_none(self):
        ...
        with (
            patch('attubot.tasks.presence.bot', mock_bot),
            patch('attubot.eggs.hatching._egg_repo', None),
            patch('attubot.tasks.presence.logger') as mock_logger,
        ):
            mock_logger.send_to_webhook = AsyncMock()
            await task.run()
