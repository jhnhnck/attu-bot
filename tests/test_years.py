...
from attubot.models import YearDocument  # noqa: E402
from attubot.years import Year  # noqa: E402
from tests.conftest import TEST_GUILD  # noqa: E402

# --- Helpers ---

def make_year_doc(guild=TEST_GUILD, year=1, start_time=1704067200, end_time=1705276800, duration=14, formatted='# === Year 1 PC ==='):
    """Create a YearDocument for testing"""
    return YearDocument(
...
def make_year(guild=TEST_GUILD, year=1, start_time=1704067200, end_time=1705276800, duration=14, formatted='# === Year 1 PC ==='):
    """Create a Year instance for testing"""
    return Year(
        guild=guild, year=year, start_time=start_time,
        end_time=end_time, duration=duration, formatted=formatted,
    )
...
@pytest.fixture
def mock_repo():
    """Fixture that patches the _get_repo() function with an AsyncMock repository"""
    repo = AsyncMock()
...
class TestToSpan:
    def test_returns_attu_year_span(self):
    ...
    def test_span_current_year_no_end(self):
...
class TestNavigation:
    @pytest.mark.asyncio
    async def test_prev_returns_previous_year(self, mock_repo):
        mock_repo.get = AsyncMock(return_value=make_year_doc(year=2))
        y = make_year(year=3)
        prev = await y.prev()
        assert prev is not None
        assert prev.year == 2
        mock_repo.get.assert_called_once_with(TEST_GUILD, 2)

    @pytest.mark.asyncio
    async def test_prev_year_one_returns_none(self, mock_repo):
        y = make_year(year=1)
        prev = await y.prev()
        assert prev is None
    ...
    @pytest.mark.asyncio
    async def test_prev_not_found_returns_none(self, mock_repo):
    ...
    @pytest.mark.asyncio
    async def test_next_returns_next_year(self, mock_repo):
        mock_repo.get = AsyncMock(return_value=make_year_doc(year=4))
        y = make_year(year=3)
        nxt = await y.next()
        assert nxt is not None
        assert nxt.year == 4
        mock_repo.get.assert_called_once_with(TEST_GUILD, 4)

    @pytest.mark.asyncio
    async def test_next_not_found_returns_none(self, mock_repo):
        mock_repo.get = AsyncMock(return_value=None)
        y = make_year(year=10)
        nxt = await y.next()
        assert nxt is None
...
class TestPersistence:
    @pytest.mark.asyncio
    async def test_save_calls_upsert(self, mock_repo):
    ...
    @pytest.mark.asyncio
    async def test_update_modifies_instance_and_persists(self, mock_repo):
        y = make_year(year=3, end_time=0, duration=0)
        await y.update(end_time=999, duration=10)
        assert y.end_time == 999
        assert y.duration == 10
        mock_repo.update.assert_called_once_with(TEST_GUILD, 3, end_time=999, duration=10)

    @pytest.mark.asyncio
    async def test_update_ignores_unknown_fields(self, mock_repo):
        y = make_year()
    ...
    @pytest.mark.asyncio
    async def test_delete_calls_repo(self, mock_repo):
        y = make_year(year=7)
        await y.delete()
        mock_repo.delete.assert_called_once_with(TEST_GUILD, 7)
...
class TestClassMethods:
    @pytest.mark.asyncio
    async def test_get_found(self, mock_repo):
        mock_repo.get = AsyncMock(return_value=make_year_doc(year=3))
    ...
    @pytest.mark.asyncio
    async def test_get_not_found(self, mock_repo):
        mock_repo.get = AsyncMock(return_value=None)
        result = await Year.get(TEST_GUILD, 99)
        assert result is None

    @pytest.mark.asyncio
    async def test_total(self, mock_repo, guild):
        mock_repo.total = AsyncMock(return_value=42)
        result = await Year.total(TEST_GUILD)
        assert result == 42
        mock_repo.total.assert_called_once_with(TEST_GUILD)

    @pytest.mark.asyncio
    async def test_total_uses_primary_guild(self, mock_repo, guild):
        mock_repo.total = AsyncMock(return_value=5)
        result = await Year.total()
        assert result == 5
        mock_repo.total.assert_called_once_with(TEST_GUILD)

    @pytest.mark.asyncio
    async def test_exists_true(self, mock_repo):
        mock_repo.exists = AsyncMock(return_value=True)
        assert await Year.exists(TEST_GUILD, 1) is True

    @pytest.mark.asyncio
    async def test_exists_false(self, mock_repo):
        mock_repo.exists = AsyncMock(return_value=False)
        assert await Year.exists(TEST_GUILD, 99) is False

    @pytest.mark.asyncio
    async def test_all_for_guild(self, mock_repo):
        mock_repo.all_for_guild = AsyncMock(return_value=[
    ...
    @pytest.mark.asyncio
    async def test_all_for_guild_empty(self, mock_repo):
        mock_repo.all_for_guild = AsyncMock(return_value=[])
        results = await Year.all_for_guild(TEST_GUILD)
        assert results == []

    @pytest.mark.asyncio
    async def test_get_latest(self, mock_repo):
        mock_repo.get_latest = AsyncMock(return_value=make_year_doc(year=10))
        result = await Year.get_latest(TEST_GUILD)
        assert result is not None
        assert result.year == 10

    @pytest.mark.asyncio
    async def test_get_latest_empty(self, mock_repo):
...
class TestLifecycle:
    @pytest.mark.asyncio
    async def test_create_from_rollover(self, mock_repo):
    ...
    @pytest.mark.asyncio
    async def test_create_from_rollover_formatted_matches_format_year_line(self, mock_repo):
    ...
    @pytest.mark.asyncio
    async def test_finalize_sets_end_time_and_duration(self, mock_repo):
        start = 1704067200
        end = start + (14 * SECONDS_PER_DAY)  # 14 days later
        mock_repo.get = AsyncMock(return_value=make_year_doc(year=3, start_time=start, end_time=0, duration=0))

        result = await Year.finalize(TEST_GUILD, 3, end_time=end)
        assert result is not None
        assert result.end_time == end
        assert result.duration == 14
        mock_repo.update.assert_called_once_with(TEST_GUILD, 3, end_time=end, duration=14)

    @pytest.mark.asyncio
    async def test_finalize_nonexistent_returns_none(self, mock_repo):
        mock_repo.get = AsyncMock(return_value=None)
    ...
    @pytest.mark.asyncio
    async def test_finalize_rounds_duration(self, mock_repo):
        """Duration should be rounded to nearest day"""
        start = 1704067200
        end = start + int(14.6 * SECONDS_PER_DAY)  # 14.6 days
        mock_repo.get = AsyncMock(return_value=make_year_doc(year=1, start_time=start, end_time=0, duration=0))
