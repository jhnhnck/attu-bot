...
"""doom_bot.client | discord client entry point."""
...
import doom_bot.commands
from doom_bot.client.core import bot, config, db  # noqa: F401 - re-exported as client API
from doom_bot.client.embeds import ui_emoji
from doom_bot.logging import PycordBridgeHandler, get_logger
...
# auto-discovered from doom_bot/commands/; sorted for deterministic loading order
extensions_list = sorted(f'doom_bot.commands.{mod.name}' for mod in pkgutil.iter_modules(doom_bot.commands.__path__))
...
def _load_event_handlers():
    ...
    importlib.import_module('doom_bot.client.events')
    importlib.import_module('doom_bot.client.modlog')
