...
"""doom_bot.logging | logging utility."""
...
def _configure() -> None:
    ...
    if any(getattr(h, '_doom_bot_owned', False) for h in root.handlers):
        return
    ...
    stdout_h._doom_bot_owned = True  # type: ignore[attr-defined]  # marker for idempotent re-imports

    stderr_h = logging.StreamHandler(sys.stderr)
    stderr_h.setFormatter(formatter)
    stderr_h.setLevel(logging.WARNING)
    stderr_h._doom_bot_owned = True  # type: ignore[attr-defined]  # marker for idempotent re-imports
...
class AttubotLogger:
    ...
    async def send_to_webhook(self, error: Exception, location: str = '') -> None:
        from doom_bot.webhook import send_to_webhook
