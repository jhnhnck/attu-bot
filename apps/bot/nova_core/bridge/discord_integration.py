...
async def get_guild_channels(guild_id: int) -> list[dict[str, Any]] | None:
    ...
    try:
        ...
        logger.exception(f'failed to fetch channels for guild {guild_id} via pycord')
        return None
...
async def get_guild_roles(guild_id: int) -> list[dict[str, Any]] | None:
    ...
    try:
        ...
        logger.exception(f'failed to fetch roles for guild {guild_id} via pycord')
        return None
...
async def get_user_info(user_id: int) -> dict[str, Any] | None:
    ...
    try:
        ...
        logger.exception(f'failed to fetch user {user_id} via pycord')
        return None
...
async def get_guild_info(guild_id: int) -> dict[str, Any] | None:
    ...
    try:
        ...
        logger.exception(f'failed to fetch guild {guild_id} via pycord')
        return None
