...
"""nova_core.commands.stars | stars commands."""
...
def _get_msg_repo():
    from nova_core.client.messages import _get_repo

    return _get_repo()


def _get_entry_repo():
    from nova_core.ccboard import _entry_repo
...
def _get_cc_reaction_repo():
    from nova_core.ccboard import _reaction_repo
...
async def _show_ccboard_random(ctx: ApplicationContext, guild_id: int, guild_config, *, min_positive: int, max_positive: int | None = None) -> None:
    ...
    from nova_core.ccboard.builder import build_embeds as cc_build_embeds
