...
## phase 2 retro - 2026-08-29

**what landed vs spec:** full dod met, integration-check passed with no findings. `.claude/settings.json` and CLAUDE.md's sub-agents bullet updated (`1f3b21b`) with the `## rules` section confirmed untouched by direct diff against trunk. README's Tests section rewritten (`b3f31af`, `1282399`) covering both paths, per-suite requirements, the `ATTU_CONFIG_FILE`/`TEST_DB_URL` distinction, the worktree workaround, and the `mongo:8` reliability caveat. mock-compensation's five `doom_bot` references repointed to `nova_core` (`476410e`), plus two items folded in beyond the literal brief: the `test_start_bot_loop.py` line reference re-verified and corrected (`:350` -> `:360`, the actual method line, not the class line) and a dead link to the deleted `docs/dev/testing.md` repointed to `README.md#tests` (`6c4f312`). one scope call: the plan's line asking CLAUDE.md to "note explicitly" the worktree-run instruction was treated as satisfied by the landed wording rather than requiring a second edit - intentional narrowing, documented inline at the time (see phase 2 verification note above), not a gap.

**what surprised us:** no separate implementer notes were relayed beyond what's already logged inline during the phase - the main agent ran this phase directly (not dispatched, per CLAUDE.md's docker/pytest sub-agent restriction) and recorded surprises as they happened: the mock-compensation line reference was wrong by 10 lines (pointed at the class, not the method) and only caught by direct re-read rather than assumed correct; and a dead link to a deleted doc surfaced adjacent to the edit already in flight and got picked up as a small, directly-adjacent fix rather than deferred. the `TEST_DB_URL`-vs-`database.url` precedence question carried in from phase 0 resolved cleanly by reading `config.py` - no live db needed, no surprise in the answer itself (`DatabaseConfig` has zero env-var awareness), but it closed out debt that had been open since phase 0's retro.

**what residual debt remains:** none new from phase 2 itself. carried forward, all previously triaged and none blocking: `mongo:8` container segfault (needs a user decision - try `mongo:7`, check for a known issue, or accept short-run-only reliability), docker-path integration test gap (deferred, standalone item-slice fodder; its phase-2 relevance - documenting docker's real limits - is now discharged), `.env`'s dangling `ATTU_CONFIG_FILE` (deferred, standalone item-slice fodder), `uv.lock`/vulture lockfile drift (deferred, pre-existing and repo-wide). the mongo-cutover cross-plan pending edit (its phase 1 scope line needs "add mongo service" -> "extend the existing mongo service" once this plan merges) remains open and intentionally un-actioned here, per this plan's own constraints - flagged for the user at merge time.

**plan overall state (last phase):** all three phases (0, 1, 2) are complete and dod-met, stacked on one branch (`worktree-venv-tests`), zero merges to trunk this run per explicit user instruction ("run it all. no merges yet"). shipped, unmerged: `mongo` service in dev compose; `run_tests.py` runs all suites directly from venv with no docker relaunch and a corrected fallback URL; `CLAUDE.md`/`settings.json` updated to allow the venv runner; README's Tests section rewritten; mock-compensation repointed to `nova_core`. still open, none blocking: the `mongo:8` segfault decision, the mongo-cutover pending scope-line edit, the docker-path integration gap, the `.env` misconfiguration, and `uv.lock`/vulture drift - all parked or flagged for the user, not for this plan to resolve. per this run's curtailed shipdown, only the final integration-check and ship-readiness pass happen next; pre-merge (including `docs/to-do.md` promotion) and the actual merge are both deferred until the user gives explicit go-ahead in a future turn.

## revision after phase 2 - 2026-08-29

bug-triage: 5 entries reviewed (1 gc'd to `## closed`, 4 remain open). the `TEST_DB_URL`-vs-`database.url` precedence entry moved to a new `## closed` section, citing `6610b61` (the commit that resolved it) - text kept verbatim, not truncated, since `plan.md`'s phase 2 line still points readers at `bugs.md` for the evidence. the `mongo:8` segfault entry (previously "needs a decision" with no formal disposition line) got one added: defer, matching the other entries' format - not a reclassification, formalizing the state integration-check already found sensible. the docker-path integration gap entry got a one-line addendum noting its phase-2 relevance is discharged (README now documents docker as unit/component-only) while the underlying gap itself stays deferred. `uv.lock`/vulture drift and the `.env` misconfiguration entries reviewed, unchanged - already correctly dispositioned as defer from phase 0's triage.
...
## mongo:8 crash root-caused and fixed - 2026-08-29

user pointed at a specific lead ("github says GLIBC_TUNABLES=\"glibc.pthread.rseq=1\" is the right fix") and asked to investigate via `/bug-hunt` rather than continuing ad hoc. before dispatching, a fresh idle-crash reproduction happened on its own: `docker inspect` showed `StartedAt: 04:30:40`, `FinishedAt: 04:31:09` - dead 21s after a clean startup, zero test load applied this session yet, same signature as crash 3 (normal startup log, then total silence, then death). this made the bug trivially reproducible on demand.

ran `.claude/worktrees/venv-tests:bug-hunt` (the directory-scoped variant) with both crash signatures laid out separately. websearch + `gh api` against `docker-library/mongo` discussions #722 and #748 confirmed the user's lead precisely: `mongo:8`'s image bakes in `GLIBC_TUNABLES=glibc.pthread.rseq=0` (added upstream for an unrelated tcmalloc perf warning, discussion #722); on host kernels >=6.19 that setting trips a kernel-level rseq bug (now confirmed upstream, linked kernel-list thread) that segfaults mongod ~30s after startup even fully idle - exact match for discussion #748's "hard-crashes exactly every 30 seconds" report, same mongod version family (8.2.x), same log signature. confirmed via `docker image inspect mongo:8` (`GLIBC_TUNABLES=glibc.pthread.rseq=0` baked in) and `uname -r` (host kernel `7.0.0-3-pve`, past the 6.19 threshold). the earlier shadow-stack guess (`glibc.cpu.hwcaps=-SHSTK`) is now understood as accidentally-half-right: per discussion #748's own findings, that env var *replaces* the whole `GLIBC_TUNABLES` string, incidentally wiping out the baked-in `rseq=0` (glibc's default for that tunable is `1`) - which likely masked the idle crash in earlier testing while doing nothing for the separate load crash, explaining why that full-suite re-test still failed with the same exit code for a different reason.

applied both fixes together in `docker-compose.dev.yml`: `GLIBC_TUNABLES: glibc.pthread.rseq=1` (idle/rseq crash) alongside the existing `ulimits: nofile: 64000` (load/EMFILE crash, from earlier this session). verified empirically, not assumed: container recreated, both settings confirmed active inside it; left running idle for 90s past the point every previous idle crash had already killed it - survived (`Status: running`); then ran the full `tests/python/component/` suite (166 tests, `-m component`, the same collection/index churn that caused the original EMFILE panic) - completed in 32.63s, mongo `Status: running` throughout, no crash. 11 failures remain, all pre-existing `ImportError`s (`fix_ccboard_recount`/`fix_ccboard_cleanup`/`fix_starboard_purge` missing from `nova_core.commands.fix`) unrelated to mongo - not investigated further, out of scope for this fix.

committed `dfe7a90` (`docker-compose.dev.yml` only; `uv.lock`'s pre-existing drift discarded from the diff as usual). `bugs.md`'s mongo entry moved from "needs a decision" to `## closed`, with the two-root-cause breakdown. `ship.md`'s deferred list updated: the `[high]` mongo entry removed, replaced with a `### resolved since original ship pass` note. no plan status change - all three phases were already "pending merge"; this was a fix to a deferred item, not new phase work. no merge action taken - "no merges yet" remains in force, untouched by this fix.

## shipdown - pre-merge and final merge - 2026-09-03

user gave the go-ahead ("merge venv-tests"). rebased `worktree-venv-tests` onto trunk (`196ccfe`, 4 commits past the branch point) - clean, no conflicts; all 23 commits rewritten, so the status table's per-phase shas above are the post-rebase retro commits (90460b4, 11e8045, 0d250c7).

s4 pre-merge: two deferred items migrated to `docs/bugs.md` (docker-path integration test gap -> testing; `.env` `ATTU_CONFIG_FILE` misconfiguration -> deploy). `uv.lock` drift dropped - `uv lock --check` is clean on trunk now. the mongo-cutover cross-plan edit applied: phase 1's compose scope line now reads "extend the existing `mongo` service" and lists the venv-tests settings to keep. no durable doc promotion needed - the rseq/ulimit findings already live as comments on the compose service and the `TEST_DB_URL` vs `database.url` split is in README. no stale cross-references to this plan outside its own dir. plan dir moved to `.claude/plans/archived/venv-tests/` per the mkdocs convention.

s5: fast-forward merge into trunk, worktree and branch pruned. README's "known issue, still open" note about the mongo:8 crash is now stale (fixed in the rseq/ulimit commit) - handled in the follow-on readme update, not here.
