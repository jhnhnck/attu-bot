...
- **design nit: `wire_wiki_command_repo` in repositories.py wires nova_core.commands.wiki** — creates a `nova_core.wiki.repositories` → `nova_core.commands` cross-module dep. eggs/ccboard keep this wiring in `__init__.py`. forced by 30-line shim constraint (phase 3 dod). low risk; move wiring into `nova_core/wiki/__init__.py` in a future cleanup when the line-count concern can be revisited.
