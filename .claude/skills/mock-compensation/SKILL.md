...
description: AttuBot's mock compensation rule - when a test mocks a framework mechanism, it owes a real test elsewhere that exercises the mocked behavior. trigger when editing or creating files under tests/python/, when adding new files under nova_core/commands/, when adding new predicates to nova_core/client/util.py, or when adding new migrations to nova_core/client/migrations.py.
---

# mock-compensation

**the rule:** when a test mocks a framework mechanism, it inherits responsibility for the behavior that mock hides; cover that behavior at another level or the failure mode is silent green CI. this skill is canonical; see also [`README.md`](README.md#tests) for surrounding test conventions (running tests, suite requirements).
...
- **trigger:** any new predicate added to [`nova_core/client/util.py`](nova_core/client/util.py), or any existing predicate applied to a new command — add corresponding true/false tests in `test_util.py`.
...
- **compensation:** for every new entry in [`nova_core/client/migrations.py`](nova_core/client/migrations.py), add a rollback path test in [`tests/python/unit/test_migrations.py`](tests/python/unit/test_migrations.py) before merging.
- **trigger:** any new `@migration(...)` decorator added to `nova_core/client/migrations.py`.
