---
name: file-header
description: AttuBot file header convention - SPDX license tag plus a structured breadcrumb docstring on every newly created source file. trigger when creating any new file under `apps/`, `packages/`, `tests/`, or `scripts/`; before writing the first line of a new `.py`, `.js`, `.ts`, `.sh`, `.zsh`, or `.bash` file; when using the `Write` tool to create a new source file; when answering questions about file headers, license markers, copyright notices, or SPDX in this repo. canonical source for the header format - notes/agents.md still describes the older verbose-docstring convention; when this skill diverges from agents.md, this skill wins and agents.md should be updated to match.
---

# file-header

Canonical reference when **creating a new source file** in this repo. This skill is canonical; update it directly. It supersedes the verbose docstring header described in `notes/agents.md` - flag that divergence so agents.md can be updated to match.
...
- **`wip/` and `*.wip.py` files**: excluded from linting per `notes/agents.md`. headers optional - skip them for scratch work.
...
- `notes/agents.md` "file header (required on every python file)" section - currently shows the older verbose form; flag for update when this skill is adopted
