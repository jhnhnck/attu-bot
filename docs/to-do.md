...
- ⭕ `low priority` `low effort` audit debug commands for embed/theme consistency — `/version` uses `make_embed` and bot theme; `eggs_show` and `previews` still respond with plain text (message_stats, dump_config, dump_starboard were removed in repl-commands branch)
...
- ⭕ `low priority` `medium effort` the TT era is handled in exactly one place: `client/calendar.py` `haracalnde_date()` renders `{1 - year} TT` for years at or below 0. every other year string hardcodes ` PC`, and all the year commands reject `year < 1`, so a TT year is unreachable through any command. decide whether pre-calming years are addressable and, if so, carry the era rather than hardcoding it in `commands/year.py`, `commands/marker.py` and `tasks/nova_year.py`
...
- ⭕ `medium priority` `medium effort` add tests for nova_core/starboard/handlers.py (69%) - 168 missed statements; reaction cascade and embed rendering paths [moved from client/starboard.py in nova-w3 phase 6]
...
- 🔴 `12 April 2026` add tests for nova_core/trees/families.py (36% → 100%) and database/connection.py (59% → 100%) [families.py moved to nova_core/trees/ in nova-w3 phase 5]
