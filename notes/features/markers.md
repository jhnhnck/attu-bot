...
The system is backed by the existing `messages` collection (see `attubot/messages.py`). There is no separate runtime marker table; the `year_markers` collection only stores admin overrides.
...
`attubot/markers.py` exports `resolve_marker(guild, channel, year) -> ResolvedMarker`.
...
command stays in `attubot/commands/year.py`. The search logic is in `markers.py`.
...
## Key Functions (`attubot/markers.py`)
