...
class Migration:
    ...
    def parse_tokens_to_guild_config(self, guild_id: int, tokens: dict) -> dict:
        ...
        if isinstance(rollover_time_str, str) and ':' in rollover_time_str:
    ...
    async def migrate_system_config(self, config_version: str = '3.0.0'):
        ...
        try:
            ...
            # Parse error_log
            error_log_val = tokens.get('error_log', [0, 0])
            # Handle TOML array directly
            if isinstance(error_log_val, (list, tuple)):
                error_log = [int(x) for x in error_log_val]
            # Handle string representation
            elif isinstance(error_log_val, str):
                error_log = [int(x.strip()) for x in error_log_val.split(',')]
            else:
                error_log = [0, 0]
...
async def main():
    ...
    parser.add_argument(
        '--sqlite-path',
        default=None,
        help='Path to SQLite database file (default: ~/assets/markers.db)',
    )
    parser.add_argument(
        '--mongodb-url',
        help='MongoDB connection URL (default: from MONGODB_URL env var or mongodb://mongo:27017)',
    )
    parser.add_argument(
        '--mongodb-db',
        default=None,
    ...
    sqlite_path = args.sqlite_path or getenv('ATTU_MARKER_DB', '~/assets/markers.db')
    mongodb_url = args.mongodb_url or getenv('MONGODB_URL', 'mongodb://mongo:27017')
