...
function renderAuditLogs(logs) {
    ...
    tableBody.innerHTML = logs.map(log => `
        <tr>
            ...
            <td><span class="badge ${getLogBadgeClass(log.config_type)}">${log.config_type}</span></td>
            <td>${log.action}</td>
            <td>${log.guild_id || '-'}</td>
            <td>
                ${log.success
            ? '<span class="text-success"><i data-feather="check"></i></span>'
            : `<span class="text-danger" title="${log.error_message || 'Unknown error'}"><i data-feather="x"></i></span>`
        }
            </td>
            <td>
                <button class="btn btn-sm btn-outline-secondary" type="button" onclick='showLogDetails(${JSON.stringify(log)})'>Details</button>
            </td>
        </tr>
    `).join('');
...
function showLogDetails(log) {
    ...
    if (log.changes && log.changes.length > 0) {
                ...
                <tbody>
                    ${log.changes.map(c => `
                        <tr>
                            ...
                            <td class="text-danger">${c.old_value !== null ? JSON.stringify(c.old_value) : '<i>null</i>'}</td>
                            <td class="text-success">${c.new_value !== null ? JSON.stringify(c.new_value) : '<i>null</i>'}</td>
                        </tr>
                    `).join('')}
                </tbody>
            </table>
        ...
        changesHtml = `<div class="alert alert-danger">Error: ${log.error_message}</div>`;
    } else {
    ...
    content.innerHTML = `
        <dl class="row">
            <dt class="col-sm-3">Timestamp</dt><dd class="col-sm-9">${log.timestamp_formatted}</dd>
            <dt class="col-sm-3">IP Address</dt><dd class="col-sm-9">${log.ip_address}</dd>
            <dt class="col-sm-3">Action</dt><dd class="col-sm-9">${log.action} ${log.config_type}</dd>
        </dl>
        <h5>Changes</h5>
        ${changesHtml}
    `;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (guildForm) {
        const guildId = guildForm.dataset.guildId;
